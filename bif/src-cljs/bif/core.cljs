...
(def starting-state {:timer-running false
                     ...
                     :flappy-start-time 0
                     :flappy-y   start-y
                     :flappy-img "/imgs/flappy-base.png"
                     :quote nil
                     :pillar-list
                     [{ :start-time 0
                       :pos-x 900
...
(defn reset-state [{:keys [flappy-img quote]} cur-time]
  (-> starting-state
      (update-in [:pillar-list] (fn [pls] (map #(assoc % :start-time cur-time) pls)))
      (assoc
          :flappy-img flappy-img
          :quote quote
          :start-time cur-time
...
(defn main-template [{:keys [score cur-time jump-count
                             ...
                             flappy-y pillar-list flappy-img
                             quote]}]
  (sab/html [:div.board { :onMouseDown (fn [e]
             ...
             (when quote
               [:p.quote.triangle-isosceles quote])
             [:div.flappy {:style {:backgroundImage (str "url(" flappy-img ")")
                                   :top (px flappy-y)}}]
...
(go-loop []
  (when-let [[e-type e] (<! ch-chsk)]
    (when (= :chsk/recv e-type)
      (let [[_ {:keys [image quote]}] e]
        (.log js/console (str "Set flappy to " image " with: " quote))
        (swap! flap-state assoc :flappy-img image :quote quote)))
    (recur)))
